...
class _Node:
    ...
    def build(self, data, target):
        ...
        if len(np.unique(target)) == 1:
            self.label = target[0]
            # self.label += np.random.normal(0,0.1)
            return
...
class DecisionTree(object):
    ...
    def __init__(self, criterion: object = 0.1) -> object:
        """初期化処理
...
if __name__ == "__main__":
    clf = DecisionTree(criterion=10 ** -10)
    # x = np.r_[np.random.normal(2, 2, size=[100, 2]), np.random.normal(-2, 2, size=[100, 2])]
    # x = np.r_[np.random.normal(2, 2, size=[100, 2]), np.random.normal(-2, 2, size=[100, 2])]
    ...
    xx, yy = np.meshgrid(np.linspace(-7.5, 7.5, 100), np.linspace(-7.5, 7.5, 100))
    X = np.c_[np.ravel(xx), np.ravel(yy)]
    prediction = clf.predict(X)
    clf.print_tree()
    import matplotlib.pyplot as plt

    plt.pcolor(xx, yy, prediction.reshape([100, 100]))
